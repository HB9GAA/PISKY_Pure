...
int BuildSentence(unsigned char *TxLine, int Channel, struct TGPS *GPS)
	...
	if(Config.UNIXformat) 						//added by HB9GAA
		{
		time_t t = time(NULL);						//current time in seconds
		sprintf(TimeBuffer, "%li", t);		//UNIX_time
		}
	else
		{
	...
	if (ShowFields && Config.UNIXformat) 
		printf("%s: ID,Ctr,UNIX-Time,Lat,Lon,Alt,Vspeed,Hspeed,Hdir\n", Channels[Channel]);

	if (ShowFields && !Config.UNIXformat) 
		printf("%s: ID,Ctr,Time,Lat,Lon,Alt,Speed,Head,Sats,Int.Temp", Channels[Channel]);
	...
	if (Config.UNIXformat)									//We format for the HaB-Traker from HB9GAA
		{
		snprintf(Sentence, 512, "$$%.15s,%d,%.15s,%7.5lf,%7.5lf,%5.5" PRId32  ",%5.1f,%d,%d,%3.1f%.12s%.20s%.20s%.40s%.90s%.20s%.10s%.40s",
			...
			GPS->Altitude,
			GPS->AscentRate,
     (GPS->Speed * 463) / 250,							//Conversion from knot to km/h
		 	GPS->Direction,
